...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for iteration in range(self.iterations):
            for state in self.mdp.getStates():
                ...
                value_iterations[state] = max(v)
            self.values = value_iterations.copy()
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for new_state, probability in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            new_values = self.values[new_state]
            Qvalue += probability * (reward + (discount * new_values))
        return Qvalue
    ...
    def computeActionFromValues(self, state):
        ...
        best_value = -maxint
        best_action = None
        if not self.mdp.isTerminal(state) and len(actions) > 0:
